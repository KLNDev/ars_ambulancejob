...
local function checkPatient(target)
    ...
    local options = {
...

        {
            ...
            description = 'Vérifier les blessures du patient',
            icon = 'user-injured',
            onSelect = function()
                local passData = {}
    ...
    if isDead then
        options[#options + 1] = {
            ...
            description = 'Réanimation du patient, nécessite un Défibrillateur',
            icon = 'medkit',
            iconColor = "#5BC0DE",
            onSelect = function()
    ...
    if player.distressCallTime then
        ...
        if currentTime - player.distressCallTime < 60000 * Config.WaitTimeForNewCall then
            return utils.showNotification(
                "Attend avant d'appeler de nouveau")
        end
    end

    local input = lib.inputDialog('Arius Ambulance', {
        { type = 'input', label = 'Message', description = 'Entre le message à transmettre aux EMS en service', required = true },
    })
                    ...
                    options = {
                        {
                            ...
                            iconColor   = "#0077FF",
                            readOnly    = true,
                            description = "Message envoyé par le patient"
                        },
                        {
                            title       = call.location,
                            icon        = "fa-map-marker",
                            iconColor   = "#00CC00",
                            readOnly    = true,
                            description = "Localisation du patient"
                        },
                        {
                            title       = "Set Waypoint",
                            icon        = "fa-map-pin",
                            iconColor   = "#FFA500",
                            arrow       = true,
                            description = "Afficher la localisation du patient sur le GPS",

                            onSelect    = function()
                                SetNewWaypoint(call.gps.x, call.gps.y)
                                utils.showNotification("Localisation définie")
                        ...
                        },
                        {
                            ...
                            iconColor   = "#32CD32",
                            arrow       = true,
                            description = "Signalé l'appel comme résolu",
                            onSelect    = function()
                                TriggerServerEvent("ars_ambulancejob:callCompleted", call)
                                utils.showNotification("Appel Cloturé")
                                ClearPedTasks(playerPed)
                                DeleteEntity(tablet)
                            end
                        },
...
addGlobalPlayer({
    ...
    --------------------AJOUT------------------------
    {
        ...
        onSelect = function(data)
            invoice(type(data) == "number" and data or data.entity)
        end
    },
    -------------------------------------------------
    {
...
RegisterNetEvent("ars_ambulancejob:playHealAnim", function(data)
    ...
    if data.anim == "medic" then
        utils.showNotification("Réanimation du patient")
    ...
    elseif data.anim == "dead" then
        utils.showNotification("Vous êtes réanimé")
...
RegisterNetEvent("ars_ambulancejob:createDistressCall", function(name)
    ...
    lib.notify({
        title = "Nouvel appel de détresse",
