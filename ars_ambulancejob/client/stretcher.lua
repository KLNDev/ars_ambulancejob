...
local function useStretcher(stretcher)
    ...
    lib.showTextUI('Brancard sorti : [G] Déposer', {
...
-------------------------------MODIF-----------------------------------------
    if not currentVehicle then
        local seats = GetVehicleMaxNumberOfPassengers(vehicle)
        local seatToGo = nil -- Initialise seatToGo à nil pour le moment

        for i = 2, seats - 1 do -- Commence à 2 pour le siège 3, car les sièges sont généralement indexés à partir de 0
            if IsVehicleSeatFree(vehicle, i) then
                if i == 2 or i == 3 then -- Vérifie si le siège est le siège 3 ou 4
                    seatToGo = i
                    break
                end
            end
