        ...
        while player.isDead do
            ...
            if not player.gettingRevived and not player.respawning then
                ...
                utils.drawTextFrame({
                    x = 0.5,
                    y = 0.9,
                    msg = "Appuyez sur ~r~E~s~ pour appeler les EMS"
                })
                ...
                if GetGameTimer() - deathTime >= time then
                    ...
                    utils.drawTextFrame({
                        x = 0.5,
                        y = 0.86,
                        msg = "Appuyez sur ~r~G~s~ pour appeler l'unité X"
                    })

                    if IsControlJustPressed(0, 47) then
                        local confermation = lib.alertDialog({
                            header = 'Unité X',
                            content = "Voulez-vous vraiment appeler l'unité X",
                            centered = true,
                            cancel = true
                        })
                    ...
                    utils.drawTextFrame({
                        x = 0.5,
                        y = 0.86,
                        msg = ("Vous pourrez appeler l'unité X dans ~b~ %s secondes~s~"):format(math.floor((time / 1000) - elapsedSeconds))
                    })
                end
            end
